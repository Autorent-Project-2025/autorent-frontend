<template>
  ...
  >
      ...
      <div v-else-if="car" class="space-y-8">
        ...
        <div class="grid lg:grid-cols-2 gap-8">
          ...
          <div class="flex flex-col gap-6 h-full">
            ...
            <!-- Book Button -->
            <button
              @click="openLandlordSelectionModal"
    ...
    <!-- Landlord Selection Modal (1st) -->
    <LandlordSelectionModal
      v-if="car && isLandlordSelectionModalOpen"
      :is-open="isLandlordSelectionModalOpen"
      :car="car"
      @close="closeLandlordSelectionModal"
      @select="handleLandlordSelect"
    />

    <!-- Booking Modal (2nd - after landlord selected) -->
    <BookingModal
      v-if="car && isBookingModalOpen && selectedLandlord"
...
import LandlordSelectionModal from "../components/LandlordSelectionModal.vue";
...
const isLandlordSelectionModalOpen = ref(false);
...
// Landlord Selection Modal (Step 1)
function openLandlordSelectionModal() {
  // Проверка авторизации
  if (!isAuthenticated.value) {
    error("Войдите в систему, чтобы забронировать автомобиль");
    router.push("/login");
    return;
  }
  
  isLandlordSelectionModalOpen.value = true;
}

function closeLandlordSelectionModal() {
  isLandlordSelectionModalOpen.value = false;
}

function handleLandlordSelect(landlord: Landlord) {
  selectedLandlord.value = landlord;
  closeLandlordSelectionModal();
  // После выбора арендодателя открываем модалку бронирования
  setTimeout(() => {
    openBookingModal();
  }, 300);
}
...
async function handleBookingConfirm(startDate: string, endDate: string) {
  if (!car.value || !selectedLandlord.value) return;

  try {
    // TODO: Отправить landlordId на бэк вместе с бронированием
    await createBooking(car.value.id, startDate, endDate);
    success(
      `${car.value.brand} ${car.value.model} успешно забронирован у ${
        selectedLandlord.value.companyName || selectedLandlord.value.username
      }!`
    );
    closeBookingModal();
  } catch (e) {
